...
from unlearnable_gen_oneclass import  unlearnable_optim_linf, adv_linf
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='p2014009')# physionet MI2014001
    ...
    if args.postmask == ['optim_linf']:
        args.nmodel = 1
    # ========================model path=======================
    model_path = f'/data1/cxq/model_id/{args.dataset}/one_cla_{args.model}/'
    ...
    log_name = os.path.join(log_path,
                            f'one_cla_{args.dataset}_{args.model}_{args.maskamp}-{args.maskl2_ind}-{args.postmask}_{args.AT_eps}-{args.train}_base.log')
    

    excel_path = f'/home/xqchen/attack_id_eegn/result/excel'
    if not os.path.exists(excel_path):
        os.makedirs(excel_path)
    excel_name = os.path.join(excel_path,f'one_cla_{args.dataset}_{args.model}_{args.maskamp}-{args.maskl2_ind}-{args.postmask}_{args.AT_eps}-{args.train}_base.xlsx')
    ...
    recorder = np.zeros((round((args.repeat+2)*subject_num_dict[args.dataset]),round(4*len(args.train)*len(args.postmask))))
    recorder_pert = np.zeros((round((args.repeat+2)*subject_num_dict[args.dataset]),round(2*len(args.postmask))))
    ...
    for r in range(args.repeat):
        ...
        for p,pm in enumerate(args.postmask):
            ...
            for sub in range(subject_num_dict[args.dataset]):

                unlearn_class = np.unique(s_train)[:round(sub+1)]
                ...
                if pm == 'rand':
                    ...
                    for k in unlearn_class[-1:]:
                        ...
                        templ = torch.std(x_train[s_train==k]) * torch.FloatTensor(np.tile(np.random.uniform(0.5,1.5,x_train.shape[-2])[:,None],(x_train[s_train==k].shape[0],1,1,1))) * templ * args.maskamp
                        l2 += torch.sqrt(torch.sum(templ**2,dim=(1,2,3))).mean().numpy()
                        l_infs.append(torch.max(torch.abs(templ)).numpy())
                        x_train[s_train==k] = x_train[s_train==k] + templ.type_as(x_train)
                    linf = np.mean(np.array(l_infs))
                    l2 = l2/1
                    logging.info('max perturbation: {:.4f}, l2: {:.4f}'.format(linf,l2))
                elif pm == 'optim_linf':
                    templ = unlearnable_optim_linf(x_train, s_train, torch.std(x_train[s_train==unlearn_class[-1]]) * args.maskamp, unlearn_class[-1], args)

                    l2 = torch.mean(torch.sqrt(torch.sum((templ[s_train==unlearn_class[-1]]-x_train[s_train==unlearn_class[-1]]).reshape(len(x_train[s_train==unlearn_class[-1]]),-1)**2,axis=-1))).numpy()
                    linf = torch.max(torch.abs(templ[s_train==unlearn_class[-1]]-x_train[s_train==unlearn_class[-1]])).numpy()
                    
                    logging.info('max perturbation: {:.4f}, l2: {:.4f}'.format(linf,l2))
                    x_train = templ
                elif pm == 'adv_linf':
                    templ = adv_linf(x_train, s_train, torch.std(x_train[s_train==unlearn_class[-1]]) * args.maskamp, unlearn_class[-1], args)
                    l2 = torch.mean(torch.sqrt(torch.sum((templ[s_train==unlearn_class[-1]]-x_train[s_train==unlearn_class[-1]]).reshape(len(x_train[s_train==unlearn_class[-1]]),-1)**2,axis=-1))).numpy()
                    linf = torch.max(torch.abs(templ[s_train==unlearn_class[-1]]-x_train[s_train==unlearn_class[-1]])).numpy()
                    logging.info('max perturbation: {:.4f}, l2: {:.4f}'.format(linf,l2))
                    x_train = templ
                if pm == 'no':
                    recorder_pert[r*subject_num_dict[args.dataset]+sub,2*p] = 0
                    recorder_pert[r*subject_num_dict[args.dataset]+sub,2*p+1] = 0
                else:
                    recorder_pert[r*subject_num_dict[args.dataset]+sub,2*p] = linf
                    recorder_pert[r*subject_num_dict[args.dataset]+sub,2*p+1] = l2
                ...
                for t,tra in enumerate(args.train):
                    ...
                    recorder[r*subject_num_dict[args.dataset]+sub, p*4*len(args.train)+t*4] = task_bca
                    recorder[r*subject_num_dict[args.dataset]+sub, p*4*len(args.train)+t*4+1] = pid_bca
                    recorder[r*subject_num_dict[args.dataset]+sub, p*4*len(args.train)+t*4+2] = un_bca
                    recorder[r*subject_num_dict[args.dataset]+sub, p*4*len(args.train)+t*4+3] = clean_bca

                    torch.cuda.empty_cache()
        

    recorder_df = DataFrame(recorder,
               index = pd.MultiIndex.from_product([[f'repeat_{g}' for g in range(args.repeat)]+['avg','std'],[f'add_person_{g}' for g in range(subject_num_dict[args.dataset])]]),
