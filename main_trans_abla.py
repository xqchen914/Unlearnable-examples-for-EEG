...
def adv_test(modelF, modelC,
    ...
    if attack == 'FGSM':
        adv_x = attack_lib.FGSM_cha(nn.Sequential(modelF, modelC),
                                x,
                                y,
                                eps=eps)
    elif attack == 'PGD':
        adv_x = attack_lib.PGD_cha(nn.Sequential(modelF, modelC),
                                x,
                                y,
                                eps=eps,
                                alpha=eps / 10,
        ...
        adversary = attack_lib.AutoAttack(nn.Sequential(modelF, modelC), norm=distance,seed=seed, eps=eps,
            version='',attacks_to_run=['apgd-ce'],device=args.device,n_iter=20)
    ...
    adv_loss, adv_acc, adv_bca = eval(modelF,modelC, criloss,
         adv_loader)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='p2014009')# physionet MI2014001 p2014009
    
    parser.add_argument('--premask', type=str, default='no')#['no','rand','sn']  'optim_linf','adv_linf','optim_l2','adv_l2' 

    parser.add_argument('--postmask', nargs='+',default=[ 'sn', 'optim_linf', 'adv_linf'])# 0.1_['no', 'rand','square','rand_binary','rand_binary2','optim','optim_linf','optim_l2']
    ...
    log_name = os.path.join(log_path,
                            f'abla_trans_{args.dataset}_{args.model}_{args.maskamp}-{args.maskl2_ind}-{args.postmask}_{args.num_chunks}-{args.shuffle_rate}-{args.trans}.log')
    

    excel_path = f'/home/xqchen/attack_id_eegn/result/excel'
    if not os.path.exists(excel_path):
        os.makedirs(excel_path)
    excel_name = os.path.join(excel_path,f'abla_trans_{args.dataset}_{args.model}_{args.maskamp}-{args.maskl2_ind}-{args.postmask}_{args.num_chunks}-{args.shuffle_rate}-{args.trans}.xlsx')
