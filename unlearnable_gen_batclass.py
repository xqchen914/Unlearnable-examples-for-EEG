...
def adv_linf(x, s_label,linf, unlearn_class, args):
    ...
    ind = torch.BoolTensor([True if i in unlearn_class else False for i in s_label.numpy() ])
    x = x[ind]
    s_label = s_label[ind]
    s_label = s_label - s_label.min()
    ...
    x_ori[ind] = (x + (torch.tanh(perturbation[s_label.squeeze()]) * linf).cpu()).detach()
...
def unlearnable_optim_linf(x, s_label,linf, unlearn_class, args):
    ...
    discriminator = Discriminator(
        ...
        n_subjects=np.max((len(unlearn_class),3))).to(args.device)
    ...
    x = x[ind]
    s_label = s_label[ind]
    s_label = s_label - torch.min(s_label)
    ...
    for epoch in range(100):
        for step, (batch_x, batch_s) in enumerate(data_loader):
            ...
            # perb_batch_x = batch_x + torch.tanh(perturbation[batch_s.squeeze()]) * linf
            perb_batch_x = tcf(batch_x + torch.tanh(perturbation[batch_s.squeeze()]) * linf) #* ran[batch_s.squeeze()]
